...
def add_book(d):
        ...
        psql = Postgres()
        psql.connect('NCTI_Lib')
        err = psql.execute(psql.create_insert_sql(d, 'book'))
        if err:
            print(err)
        else:
...
def get():
    url = "http://metanbi.ncl.edu.tw/iii/encore/search/C__S%28*%29+f%3Aa+b%3A07__P1__Orightresult?lang=cht&suite=pearl"
